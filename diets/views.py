...
def diet(request):
    ...
    response = {'recipes':[]}
    for tag in tags:
        url = f'https://api.spoonacular.com/recipes/random?apiKey=41fec56909474683a86fdcfa31c0f9d7&number=1&tag={tag},healthy'
...
def detail(request, id):
    session = requests.Session()
    session.headers.update({'Content-Type' : 'application/json', 'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.76 Safari/537.36'})
    url = f'https://api.spoonacular.com/recipes/{id}/information?apiKey=41fec56909474683a86fdcfa31c0f9d7'
    response = session.get(url=url).json()
    response['protein'] = re.findall(r'\d{1,}g of protein', response['summary'])[0].split()[0]
    response['fat'] = re.findall(r'\d{1,}g of fat', response['summary'])[0].split()[0]
    response['calories'] = re.findall(r'\d{1,} calories', response['summary'])[0].split()[0]
